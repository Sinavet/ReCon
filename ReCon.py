...
if st.session_state["step"] == 1:
    ...
    if uploaded_files:
        st.button("Далее", on_click=lambda: st.session_state.update({"step": 2}))
    ...
    st.button("Далее", on_click=lambda: st.session_state.update({"step": 3}))

elif st.session_state["step"] == 3:
    # 3. Обработка
    st.header("Шаг 3: Обработка")
    st.markdown(
        """
    ...
    st.button("Далее", on_click=lambda: st.session_state.update({"step": 4}))
    ...
    st.button("Начать заново", on_click=lambda: st.session_state.update({"step": 1}))
